...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // === DVOUPÓLOVÉ ŘÁDKY PRO VÝPOČET (LHS = RHS) – pouze UI vrstva, logika zápisu zůstává ===
  function setupTwoFieldCalc() {
    ...
    const formulaWrap = formulaInput ? formulaInput.parentElement : null;
    const subsWrap = substitutionInput ? substitutionInput.parentElement : null;
    const resultWrap = userAnswerInput ? userAnswerInput.parentElement.parentElement : null;

    function buildRow(wrapper, key, lhsDefault, rhsPlaceholder) {
      if (!wrapper) return;
      ...
      lhs.value = lhsDefault; lhs.placeholder = "W/F/s";
      ...
      const onChange = () => {
        if (key==="formula") formulaLiveValidate();
        else if (key==="subs") substitutionLiveValidate();
        else if (key==="result") resultLiveValidate();
      };
      lhs.addEventListener("input", onChange);
      rhs.addEventListener("input", onChange);
    }

    buildRow(formulaWrap, "formula", "W", "F * s");
    buildRow(subsWrap, "subs", "W", "např. 1000 * 2");
    if (resultWrap) {
      const holder = document.createElement("div");
      resultWrap.parentElement.insertBefore(holder, resultWrap);
      buildRow(holder, "result", "W", "číslo");
    }
  }
  ...
  function formulaLiveValidate() {
    const lhsEl = document.getElementById("formula-lhs");
    const rhsEl = document.getElementById("formula-rhs");
    const useTwo = !!(lhsEl && rhsEl);
    const unknown = getUnknownSymbolFromZapis();
    if (useTwo) {
      const lhs = (lhsEl.value||"").trim().toUpperCase();
      const rhs = (rhsEl.value||"").trim().replace(/\s+/g,"");
      let rhsOk=false, lhsOk=true;
      if (unknown==="W") { lhsOk = lhs==="W"; rhsOk = /^(F[*·]s|s[*·]F)$/i.test(rhs); }
      else if (unknown==="F") { lhsOk = lhs==="F"; rhsOk = /^W\/s$/i.test(rhs); }
      else if (unknown==="S") { lhsOk = lhs==="S"; rhsOk = /^W\/F$/i.test(rhs); }
      rhsEl.classList.remove("ring-2","ring-red-500","ring-green-500");
      if (rhs!=="") rhsEl.classList.add("ring-2", (lhsOk && rhsOk) ? "ring-green-500":"ring-red-500");
      return;
    }
    ...
    let ok = false;
    if (unknown === "W") ok = /^W=(F[*·]s|s[*·]F)$/i.test(txt);
    else if (unknown === "F") ok = /^F=W\/s$/i.test(txt);
    else if (unknown === "s") ok = /^s=W\/F$/i.test(txt);
    if (txt!=="") formulaInput.classList.add("ring-2", ok ? "ring-green-500" : "ring-red-500");
  } else if (unknown === "F") {
      ok = /^F=W\/s$/.test(txt);
    } else if (unknown === "s") {
      ok = /^s=W\/F$/.test(txt);
  ...
  function substitutionLiveValidate() {
    const lhsEl = document.getElementById("subs-lhs");
    const rhsEl = document.getElementById("subs-rhs");
    const useTwo = !!(lhsEl && rhsEl);
    const unknown = getUnknownSymbolFromZapis();
    if (useTwo) {
      const lhs = (lhsEl.value||"").trim().toUpperCase();
      ...
      let ok=false;
      if (unknown==="W") {
        const nums = rhs.split("*").map(x=>parseNum(x));
        const gF = currentProblem?.givens?.find(g=>g.symbol==="F")?.value;
        const gs = currentProblem?.givens?.find(g=>g.symbol==="s")?.value;
        ok = (lhs==="W") && nums.some(n=>almostEqual(n,gF)) && nums.some(n=>almostEqual(n,gs));
      } else { ok = lhs===unknown; }
      rhsEl.classList.add("ring-2", ok ? "ring-green-500" : "ring-red-500");
      return;
    }
    if (!substitutionInput) return;
    substitutionInput.classList.remove("ring-2","ring-red-500","ring-green-500");
    const sTxt = (substitutionInput.value || "").replace(/\s+/g, "");
    if (sTxt==="") return;
    let ok=false;
    if (unknown==="W") {
      const nums = sTxt.split("*").map(x=>parseNum(x));
      const gF = currentProblem?.givens?.find(g=>g.symbol==="F")?.value;
      const gs = currentProblem?.givens?.find(g=>g.symbol==="s")?.value;
      ok = nums.some(n=>almostEqual(n,gF)) && nums.some(n=>almostEqual(n,gs));
    } else ok = true;
    substitutionInput.classList.add("ring-2", ok ? "ring-green-500" : "ring-red-500");
  }
          }
        }
      }
  ...
  function resultLiveValidate() {
    const lhsEl = document.getElementById("result-lhs");
    const rhsEl = document.getElementById("result-rhs");
    const useTwo = !!(lhsEl && rhsEl);
    const unknown = getUnknownSymbolFromZapis();
    if (useTwo) {
      const lhs = (lhsEl.value||"").trim().toUpperCase();
      const rhs = (rhsEl.value||"").trim();
      const unit = unitSelect?.value || "J";
      rhsEl.classList.remove("ring-2","ring-red-500","ring-green-500");
      const num = parseNum(rhs);
      if (!isFinite(num)) { rhsEl.classList.add("ring-2","ring-red-500"); return; }
      if (unknown==="W") {
        const inJ = num * (unitToBaseFactor[unit] ?? 1);
        const ok = lhs==="W" && almostEqual(inJ, currentProblem.result);
        rhsEl.classList.add("ring-2", ok ? "ring-green-500" : "ring-red-500");
      } else { rhsEl.classList.add("ring-2","ring-green-500"); }
      return;
    }
    ...
    } else { userAnswerInput.classList.add("ring-2","ring-green-500"); }
  }

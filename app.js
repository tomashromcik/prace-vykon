// ====================================================================
// app.js — Fyzika: Práce a výkon (stabilní verze pro GitHub Pages)
// Zachováno: plně funkční ZÁPIS (live validace + auto převodní řádek)
// Nové: Výpočet = dvoupólové řádky (LHS = RHS) + modální shrnutí
// Opravy: spolehlivé zvýraznění tlačítek, přechody, generátor příkladů
// ====================================================================
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const mainCalcStep = document.getElementById("main-calculation-step") || vypocetStep;
  ...
  // -------------------- Režimy/úrovně (zvýraznění tlačítek) --------------------
  const modeBtns = Array.from(document.querySelectorAll('[id^="mode-"]'));
  const levelBtns = Array.from(document.querySelectorAll('[id^="level-"]'));

  modeBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      selectedMode = btn.id.includes("practice") ? "practice" : "test";
      modeBtns.forEach(b => b.classList.remove("ring-2","ring-blue-500"));
  ...
  levelBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      levelBtns.forEach(b => b.classList.remove("ring-2","ring-blue-500"));
      btn.classList.add("ring-2","ring-blue-500");
      enableStartIfReady();
  ...
  function ensureCalcRows(){
    if(document.getElementById("formula-lhs")) return; // už existují
  ...
  function createCalcRow(labelText, key, defaults){
    ...
    wrap.append(lab, row);
    mainCalcStep.insertBefore(wrap, checkCalcBtn);

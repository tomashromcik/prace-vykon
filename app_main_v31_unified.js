 ...
 * v31: kompatibilní názvy/metody (newProblem, resetZapis, fullReset, seedVypocetFields, enterVypocet),
 *      vynucení základních jednotek, askFor-aware seeding, živá validace, modály, autostart.
 */
(function(){
  'use strict';
  ...
  const App = {
    ...
    cacheEls(){
      ...
      // modals (content gets filled from code if needed)
    ...
    // ---------- setup & navigation ----------
    bindSetup(){
      const setMode = (m)=>{ this.state.mode=m; };
      const setLevel = (l)=>{ this.state.level=l; };
    ...
    newProblem(){
      ...
      // doplň zápis: základní jednotky + hledaná veličina prázdná
